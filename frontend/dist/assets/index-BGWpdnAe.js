import{A as y,a as k}from"./config-B6wYyi2L.js";import{r as E}from"./auth-By1CJaU_.js";import{o as $,a as T,b as U,e as C}from"./index-C_pcrWqH.js";const S={__name:"index",setup(_){return $(async()=>{E(["admin"]);const o=document.getElementById("alertContainer"),i=document.getElementById("tablaUsuarios"),m=document.getElementById("filtroUsuario"),x=document.getElementById("btnFiltrar"),b=document.getElementById("resumenTotal"),v=document.getElementById("resumenTelefono"),f=document.getElementById("resumenSinTelefono"),r=document.getElementById("actividadUsuarios");let a=[];const n=(s,e="danger")=>{o&&(o.innerHTML=`
      <div class="alert alert-${e} rounded-4" role="alert">
        ${s}
      </div>
    `)},p=()=>({Authorization:`Bearer ${k()}`,"Content-Type":"application/json"}),l=s=>{b&&(b.textContent=s.length),v&&(v.textContent=s.filter(e=>(e.telefono||"").trim()!=="").length),f&&(f.textContent=s.filter(e=>!(e.telefono||"").trim()).length)},d=s=>{if(!r)return;const e=s.slice(0,3);if(!e.length){r.innerHTML=`
        <div class="p-3 rounded-4 bg-light border-start border-4 border-primary text-muted">
          No hay actividad disponible.
        </div>
      `;return}r.innerHTML=e.map((t,u)=>`
      <div class="p-3 rounded-4 bg-light border-start border-4 ${u===0?"border-primary":u===1?"border-success":"border-warning"}">
    ...
    `).join("")},c=s=>{if(i){if(!s.length){i.innerHTML=`
        <tr>
